...
describe("ordenarPorPrecio", () => {
  ...
  it("por defecto ordena por precio total, aunque haya presentaciones comparables", () => {
    const resultados: Resultado[] = [
      { tienda: "A", nombre: "Aceite 250ml", precio: 6000, disponible: true },
      { tienda: "B", nombre: "Aceite 1L", precio: 15000, disponible: true },
    ];
  ...
  it("criterio 'unidad': ordena por precio por mL/g/unidad en vez de precio total", () => {
    const resultados: Resultado[] = [
      { tienda: "A", nombre: "Aceite 250ml", precio: 6000, disponible: true },
      { tienda: "B", nombre: "Aceite 1L", precio: 15000, disponible: true },
    ];
    // Por mL, A (250ml a 6000) sale más caro que B (1L a 15000): A=24/mL, B=15/mL.
    expect(ordenarPorPrecio(resultados, "unidad").map((r) => r.tienda)).toEqual([
      "B",
      "A",
    ]);
  });

  it("criterio 'unidad': no exige la misma unidad entre items, a diferencia de calcularMejor", () => {
    const resultados: Resultado[] = [
      { tienda: "A", nombre: "Aceite 1L", precio: 4000, disponible: true }, // 4/mL
      { tienda: "B", nombre: "Arroz 1kg", precio: 3000, disponible: true }, // 3/g
    ];
    expect(ordenarPorPrecio(resultados, "unidad").map((r) => r.tienda)).toEqual([
      "B",
      "A",
  ...
  it("criterio 'unidad': deja los que tienen precio pero no presentación reconocible justo antes de los sin precio", () => {
    const resultados: Resultado[] = [
      { tienda: "A", nombre: "Aceite genérico", precio: 3000, disponible: true },
      { tienda: "B", nombre: "sin-precio", precio: null, disponible: true },
      { tienda: "C", nombre: "Aceite 1L", precio: 15000, disponible: true },
    ];
    expect(ordenarPorPrecio(resultados, "unidad").map((r) => r.tienda)).toEqual([
      "C",
      "A",
