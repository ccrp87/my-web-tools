...
import type { SeasonPredominance } from "./lib/temporadas";

export interface SuggestionRow {
  plan: VacationPlan;
  season: SeasonPredominance | null;
}

interface SeasonFilterInfo {
  mode: SeasonFilterMode;
  hiddenCount: number;
  bestHiddenRestDays: number | null;
  isEmpty: boolean;
  onRevertToIndiferente: () => void;
}
...
interface SuggestionsTableProps {
  ...
  seasonFilter: SeasonFilterInfo | null;
}

const SEASON_LABELS: Record<SeasonPredominance, string> = {
  alta: "Alta",
  baja: "Baja",
  mixto: "Mixta",
};
...
export function SuggestionsTable({
  ...
  const bestScore = suggestions.reduce(
    (best, row) => Math.max(best, row.plan.score),
    0,
  );
  const isCurrentAlreadyBest =
    suggestions.find((row) => toISODate(row.plan.start) === currentISO)
      ?.plan.score === bestScore;

  const showSeasonColumn = suggestions.some((row) => row.season !== null);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {seasonFilter && seasonFilter.mode === "excluir_alta" && (
        <p className="rounded-r-lg border-l-4 border-indigo-500 bg-indigo-50 px-3 py-2 text-sm text-indigo-900 dark:bg-indigo-950 dark:text-indigo-200">
          {seasonFilter.isEmpty
            ? "No queda ninguna fecha fuera de temporada alta dentro del horizonte de búsqueda."
            : seasonFilter.hiddenCount > 0
              ? `Se ocultaron ${seasonFilter.hiddenCount} ${seasonFilter.hiddenCount === 1 ? "fecha" : "fechas"} de temporada alta (la mejor daba ${seasonFilter.bestHiddenRestDays} días de descanso seguido).`
              : "No había ninguna fecha de temporada alta entre las mejores opciones."}
          {(seasonFilter.isEmpty || seasonFilter.hiddenCount > 0) && (
            <>
              {" "}
              <button
                type="button"
                onClick={seasonFilter.onRevertToIndiferente}
                className="font-medium underline underline-offset-2 hover:no-underline"
              >
                Volver a indiferente
              </button>
              .
            </>
          )}
      ...
      {suggestions.length > 0 && (
        <div className="overflow-x-auto">
          <table className="w-full min-w-[32rem] border-collapse text-sm">
            ...
            <tbody>
              {suggestions.map(({ plan: candidate, season }) => {
                ...
                return (
                  ...
                  >
                    ...
                    {showSeasonColumn && (
                      <td className="border-b border-black/[.08] py-3 pr-3 align-top text-xs dark:border-white/[.145]">
                        {season && (
                          <span
                            className={
                              season === "alta"
