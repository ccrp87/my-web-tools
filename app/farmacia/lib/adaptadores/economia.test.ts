...
import { parsearPayloadFlight, parsearProducto } from "./economia";
...
describe("parsearPayloadFlight + parsearProducto", () => {
  it("extrae un producto de un payload de Next.js flight, resolviendo la foto por referencia", () => {
    ...
    expect(parsearProducto(producto, mapa)).toEqual({
      tienda: "La Economía",
  ...
  it("usa el precio promocional solo cuando es menor que el precio normal", () => {
    ...
    expect(parsearProducto(conPromoValida, mapa).precio).toBe(10300);
    expect(parsearProducto(conPromoInvalida, mapa).precio).toBe(17100);
  });
  ...
  it("no confunde una referencia de chunk inexistente con una imagen real", () => {
    ...
    expect(parsearProducto(producto, mapa).imagen).toBeUndefined();
  });
});
